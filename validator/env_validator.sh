...
function CheckGMSEC
{
	...
	for dir in $ld_lib_path; do
		if [[ -e $dir/libgmsec_api.${lib_ext} ]]; then
			gmsec_api_bin=$dir
			break
		fi
	done
	...
	if [[ -e $gmsec_api_bin/libgmsec_api.${lib_ext} ]]; then
		Success "Found Core GMSEC library libgmsec_api.${lib_ext} in $gmsec_api_bin"

		# Check to ensure we have the appropriate version of the GMSEC API for the
		# current architecture.
		#
		CheckArchitecture $gmsec_api_bin/libgmsec_api.${lib_ext}
		...
		Failure "Core GMSEC library libgmsec_api.${lib_ext} not found"
		result=1
	fi
}
...
function CheckJava
{
	...
	if [ ! -z $java_home ]; then
		...
		if [ ! -z $java_ver ]; then
			...
			validator=gov.nasa.gsfc.gmsec.api5.util.SystemValidator
...
{
	echo
	echo "${txtcyn}Checking Python3 setup...${txtrst}"
	...
	if [ $? -eq 0 ]; then
		...
		if [ $? -eq 0 ]; then
			Success "Python3 with version $python_ver (or later) found"

			if [ -z $PYTHONPATH ]; then
				Warning "PYTHONPATH is not defined"
			else
				Success "PYTHONPATH is defined"
			fi

			CheckDependencies ${gmsec_api_bin}/_libgmsec_python3.${lib_ext} true
		else
			Failure "Python3 with version $python_ver (or later) not found"
			result=1
		fi
	else
