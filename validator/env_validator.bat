...
call:FindInPath gmsec_api.dll
if %result% == "" (
	call:Failure "Core GMSEC library gmsec_api.dll not found while searching PATH"
	set system_setup=1
	goto:eof
) else (
	call:Success "Core GMSEC library gmsec_api.dll found using PATH"
	set GMSEC_BIN=%result%
)

call:CheckValidDLL %GMSEC_BIN%\gmsec_api.dll
...
:have_version
	...
	set validator="gov.nasa.gsfc.gmsec.api5.util.SystemValidator"
...
REM ##########################################################################
:CheckPython
call:Header "Checking Python setup..."
call:FindInPath python.exe
if !result! == "" (
	call:Warning "Python was not found"
) else (
	...
	if !errorlevel! == 0 (
		call:Success "Python with version !min_python_ver! (or later) found"
		
		if defined PYTHONPATH (
			call:Success "PYTHONPATH is defined"
		) else (
			call:Warning "PYTHONPATH is not defined"
		)
					
		echo.
		call:Header "Checking dependencies for _libgmsec_python3.pyd..."
		call:CheckDependencies %GMSEC_BIN%\_libgmsec_python3.pyd 1
	) else (
		call:Failure "Python with version !min_python_ver! (or later) not found"
	)
)
goto:eof
