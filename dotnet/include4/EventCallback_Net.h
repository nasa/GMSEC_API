...
#ifndef GMSEC_API_EVENTCALLBACK_NET_H
#define GMSEC_API_EVENTCALLBACK_NET_H
...
/// <summary>
/// This class is the abstract base class for event callbacks. A user created class, derrived
/// from this class, can be passed into RegisterEventCallback() to have user code executed
/// asynchronously when an event occurs in the connection object. Please note that because
/// users are able to create their own EventCallback class, reentrancy is not guaranteed unless
/// if they implement their own reentrancy rules. Also note that because an EventCallback can
/// be registered to multiple connections, it can be run concurrently amongst those connections.
/// Because of this, the use of a gmsec::api::util::AutoMutex is suggested to enforce thread safety. 
/// </summary>
///
/// <seealso cref="Connection::RegisterEventCallback(Connection::ConnectionEvent event, GMSEC::API::EventCallback^ cb)"/>


public ref class EventCallback abstract
{
public:
	...
	virtual void OnEvent(Connection^ conn, Status^ status, Connection::ConnectionEvent event);
...
protected:
    ...
    /// <param name="status">The status of the event being reported</param>
    /// <param name="event">The event that has occurred</param>
	void OnEventNative(gmsec::api::Connection& conn, const gmsec::api::Status& status, gmsec::api::Connection::ConnectionEvent event);
...
internal:
	...
	gmsec::api::EventCallback* GetUnmanagedImplementation();
	...
	void SetUnmanagedImplementation(gmsec::api::EventCallback* callback);


private:
	gmsec::api::EventCallback* m_impl;
};
