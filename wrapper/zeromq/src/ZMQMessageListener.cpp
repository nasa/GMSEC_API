...
ZMQMessageListener::ZMQMessageListener(ZMQConnection* zmqConnection, void** subSocket)
	: m_connectionPtr(zmqConnection),
	  m_subSocketPtr(*subSocket),
	  m_inclusionFilter(new gmsec::api5::internal::ExclusionFilter()),
	  m_doneWithLife(false),
	  m_deathLatch(new gmsec::api5::util::CountDownLatch(1))
{
}
...
void ZMQMessageListener::run()
{
	...
	while (!m_doneWithLife)
	{
		...
		gmsec::api5::util::StdThread::yield();
...
void ZMQMessageListener::recvMessage(const char* subject)
{
	...
	if (m_inclusionFilter->isSubjectExcluded(std::string(subject)))
	{
		...
		m_connectionPtr->handleMessage(&gmsecMessage, gmsecSize, &metaMessage, metaSize, subject);
	}
