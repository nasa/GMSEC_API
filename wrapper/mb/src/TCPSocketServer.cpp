...
Status TCPSocketServer::connect(int port)
{
	...
	GMSEC_I32 sockaddr_len = static_cast<GMSEC_I32>( (m_pAddr->sa_family == AF_INET6 ? sizeof(sockaddr_in6) : sizeof(sockaddr_in)) );
...
Status TCPSocketServer::read(char*& buffer, GMSEC_I32& len)
{
	return TCPSocket::read(buffer, len);
}


Status TCPSocketServer::write(const char* buffer, GMSEC_I32 len)
{
