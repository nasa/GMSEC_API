...
Status TCPSocket::read(char*& buffer, GMSEC_I32& len)
{
	Status    status;
	char      rcvBuff[MAX_BUF] = {0};
	int       rcvIndex = -1;
	GMSEC_I32 inMsgSize = 0;
	...
	do
	{
		...
		int n = recv(m_sock, rcvBuff + rcvIndex, 1, 0);
		...
		if (m_isDebug)
		{
			...
			{
#ifdef WIN32
				fprintf(stderr, "R: line: %d  sock: %lld   n: %d  index: %d   data --:%c\n",
#else
	...
	{
		if (!StringUtil::stringParseI32(rcvBuff, inMsgSize))
		{
	...
	while (inMsgSize > 0)
	{
		int n = recv(m_sock, buffer + (len - inMsgSize), inMsgSize, 0);
...
Status TCPSocket::write(const char * buffer, GMSEC_I32 len)
{
	const int TRIES = 3;
	char tBuff[MAX_BUF];
	Status status;
	...
	// send out the header
	GMSEC_I32 bytes_sent = 0;
	GMSEC_I32 buf_len = static_cast<GMSEC_I32>( StringUtil::stringLength(tBuff) );
	int tries = TRIES;
	...
	if (bytes_sent < buf_len)
	{
		...
		if (tmp != EPIPE)
		{
			GMSEC_WARNING << "error writing header [error=" << strerror(tmp) << "]";
		}
#endif
...
Status TCPSocket::fillAddress(SockAddr* saddr, const size_t saddrlen, const int port, const char* address)
{
	...
	if (getaddrinfo(address, portstr.str().c_str(), &hints, &pai) != 0  ||
	    !pai || !pai->ai_addr || (pai->ai_family != AF_INET && pai->ai_family != AF_INET6))
	{
#ifdef WIN32
		status = getLastSocketError();
#else
	...
	memcpy(saddr, pai->ai_addr, pai->ai_addrlen);
