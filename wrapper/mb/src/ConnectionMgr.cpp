...
ConnectionMgr::ConnectionMgr(TCPSocketServer* sock, MBService* mbService, MessageFactory& msgFactory)
	:
	m_sock(sock),
	...
	while (!disconnect)
	{
		...
		case CMD_SUB:
			// subscribe to a new subject
			{
				...
				m_sock->write(acksub, static_cast<GMSEC_I32>( StringUtil::stringLength(acksub) ) + 1);
			}
			break;
		...
		case CMD_PUB:
			...
			{
				Config   msgConfig;
				Message* msg = MBWire::deserialize(inBuff, buffLen, m_msgFactory, msgConfig);
