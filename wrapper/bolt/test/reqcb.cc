...
#ifdef BOLT_REPLY_FUNCTION
void myReplyHandler(SharedPacket packet, void *user)
{
	cout << "myReplyHandler invoked with user=" << user << "\n";
	packet->put(cout);
}
#else
class MyReplyHandler : public ReplyHandler
{
public:
	MyReplyHandler(Server s) : server(s) { }
	virtual void onPacket(SharedPacket packet) {
		cout << "MyReplyHandler.onPacket\n";
		packet->put(cout);
	}
private:
	Server server;
...
void run()
{
	...
	conn.setReplyHandler(new MyReplyHandler(server));
