...
using gmsec::api5::util::StdThread;
...
private:
	...
	string generateID();
	Result _subscribe(const SafeList<std::string> &patterns, bool await);
	Result ping();
	...
	SafeList<std::string> subscriptions;
	long long counter;
	string fTag;
	string replyTopic;
