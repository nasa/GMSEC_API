	...
	lastState(STATE_UNKNOWN)
{
	...
	test.setVerbosity(options.getI32("LOG-TestManaged", static_cast<int>(LogLevel::logWARNING)));
...
Result Connection::connect()
{
	...
	if (result.isValid())
	{
		...
		dispatching.setVerbosity(options.getI32("LOG-TestDispatching", static_cast<int>(LogLevel::logWARNING)));
...
Result Connection::subscribe(const string &pattern)
{
	...
	{
		Result result(ERR_STATE, "not connected");
		return result;
	}
	else if (shared->getMode() == CONNECT_RESUBSCRIBING)
	{
		// Delay to allow re-subscription of topic pattern(s), if any, to be set up
		while (shared->getMode() != CONNECT_DISPATCHING) {
			TimeUtil::millisleep(10);
		}
	}

	//TODO: Do not permit duplicate subscriptions (note core API already does this).

	SafeList<std::string> patterns;
	patterns.push_back(pattern);

	Result result = _subscribe(patterns, false);
...
Result Connection::_subscribe(const SafeList<std::string> &patterns, bool await)
{
	if (patterns.size() == 0)
	{
	...
	SafeList<std::string>::SafeListConstIterator i = patterns.begin();
	meta.setTopic(*i++);
...
packet->put(std::cout);
	...
	if (result.isValid() && await)
	{
		...
		ack.setVerbosity(options.getI32("LOG-TestAck", static_cast<int>(LogLevel::logWARNING)));
...
Result Connection::unsubscribe(const string &pattern)
{
	bool found = false;
	// <algorithm> find
	for (SafeList<std::string>::SafeListIterator i = subscriptions.begin();
...
{
	gmsec::api5::util::Rawbuf<64> buffer;
...
{
	SafeList<std::string> patterns = subscriptions;
...
bool Connection::isSubscribed(const string &topic)
{
	...
	if (!gmsec::api5::util::Subject::getElements(topic, elements))
	{
	...
	for (SafeList<std::string>::SafeListIterator i = subscriptions.begin();
			...
			++i)
	{
		if (gmsec::api5::util::Subject::doesSubjectMatchPattern(elements, *i))
		{
			return true;
		}
	}
