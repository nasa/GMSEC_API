...
{
	gmsec::api5::util::Rawbuf<256> buffer;
	if (!info)
		info = "Shared";
...
{
	return (State) gmsec::api5::util::syncAddAndFetch(&fState, 0);
}
...
GMSEC_VERBOSE << fTag.c_str() << "connect";
	...
	{
		gmsec::api5::util::syncAddAndFetch(&fIoCounter, 1);
...
GMSEC_VERBOSE << fTag.c_str() << "disconnect";
	...
	{
		gmsec::api5::util::syncAddAndFetch(&fTraps, 1);
	...
	{
		GMSEC_VERBOSE << fTag.c_str() << "disconnect failed " << result;
	}
...
Result Shared::put(const SharedPacket &packet)
{
	...
	{
		result.set(ERR_CONNECT, "Shared.put: not connected");
	}
	else
	{
	...
	if (fState == STATE_CONNECTED)
	{
		...
		else
		{
			...
			if (fNullRead_ms >= fNullReadLog_ms)
			{
				...
				GMSEC_VERBOSE << "inactivity: nothing read for " << fNullRead_ms << " [ms]";
			}
...
Result Shared::getInput(SharedPacket &packet, int timeout_ms)
{
	...
	if (timeout_ms > 0)
	{
		...
		test.setVerbosity(fOptions.getI32("LOG-TestInput", static_cast<int>(LogLevel::logNONE)));
...
GMSEC_DEBUG << fTag.c_str() << "handle: ACK ID=" << meta->getID().c_str();
		...
		else if (type == PACKET_REPLY && fReplyHandler)
		{
			fReplyHandler->onPacket(packet);

			expose = true;
		}
		else if (type == PACKET_PUBLISH || type == PACKET_REQUEST)
		{
			expose = true;
		}
...
{
	traps = gmsec::api5::util::syncAddAndFetch(&fTraps, 0);
	return gmsec::api5::util::syncAddAndFetch(&fIoCounter, 0);
}
