...
public:
	...
	bool asFlag() const {
		if (gmsec::api5::util::StringUtil::stringEqualsIgnoreCase(value.c_str(), "false"))
			return false;
		if (value == "0")
			return false;
		if (gmsec::api5::util::StringUtil::stringEqualsIgnoreCase(value.c_str(), "no"))
			return false;
		return true;
	}
	i32 asInteger() const {
		i32 tmp = 0;
		if (gmsec::api5::util::StringUtil::stringParseI32(value.c_str(), tmp))
			return tmp;
	...
	f64 asReal() const {
		GMSEC_F64 tmp = 0;
		if (gmsec::api5::util::StringUtil::stringParseF64(value.c_str(), tmp))
			return tmp;
...
public:
	...
	string asString() const {
		gmsec::api5::util::Rawbuf<32> buffer;
		buffer.stream() << value;
		return buffer.str();
	}
	bool asFlag() const {
...
i32 Property::getEncoding(EncMode &etype, int &prefix) const
{
	...
	if (isNamed())
		count += static_cast<bolt::i32>(name.length());
...
bool Property::put(OutputBuffer &buffer) const
{
	...
	if (isNamed())
	{
		buffer.put1(static_cast<bolt::i32>(name.length()));
		buffer.put(name);
	}
	else
		buffer.put1(0);
