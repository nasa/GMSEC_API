...
Result TCPSocket::connect (const char *host, int port)
{
	...
	string portstr = util::toString( i32(port) );

	hints.ai_family   = AF_UNSPEC;     // Allow IPv4 or IPv6
	hints.ai_socktype = SOCK_STREAM;

	int code = getaddrinfo(host, portstr.c_str(), &hints, &pai);
	if (code || !pai)
	{
		string text(host);
		text += ": ";
	...
	for (rp = pai; rp != NULL; rp = rp->ai_next)
	{
		if (!handle)
		{
			result = open(rp->ai_family);
			if (result.isError())
			{
				goto cleanup;
			}
		...
		{
			q = ::connect(handle, pai->ai_addr, static_cast<int>(pai->ai_addrlen));
			if (q < 0)
			{
				if (blocking)
				{
		...
		if (!blocking)
		{
			if (!isConnected)
			{
				...
				{
					result.set(ERR_CONNECT, "Wait for connection failed (is Bolt running?)");
				}
			}
		...
		if (isConnected)
		{
			...
			{
				q = 1;
				if (setsockopt(handle, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&q), sizeof(q)) < 0)
				{
			...
			{
				if (setsockopt(handle, SOL_SOCKET, SO_SNDBUF, reinterpret_cast<const char*>(&m_tcpBufferSize), sizeof(m_tcpBufferSize)) < 0)
				{
					...
					getsockopt(handle, SOL_SOCKET, SO_SNDBUF, reinterpret_cast<char*>(&buf_size_set), &set_size);
					GMSEC_DEBUG << "Actual send buffer size is " << buf_size_set;
				}

				if (setsockopt(handle, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<const char*>(&m_tcpBufferSize), sizeof(m_tcpBufferSize)) < 0)
				{
					...
					getsockopt(handle, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<char*>(&buf_size_set), &set_size);
...
Result TCPSocket::open(int family)
{
	Result result;
...
int TCPSocket::doRead (char *buffer, int count, int timeout_ms)
{
	...
	int q = select(static_cast<int>(handle)+1, &fdread, 0, &fderr, &tv);
	if (q == 0) // timeout
		return q;
...
int TCPSocket::write (const char *buffer, int count)
{
	...
	do {
		...
		int q = select(static_cast<int>(handle)+1, 0, &fdwrite, &fderr, &tv);
...
int TCPSocket::setError (int localError, bool setString)
{
	...
	if (setString)
	{
		gmsec::api5::util::SystemUtil::getErrorString(localError, errorString);
	}
...
int initialize()
{
	...
	{
		string s;
		gmsec::api5::util::SystemUtil::getErrorString(code, s);
