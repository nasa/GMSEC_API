	  ...
	  m_domainId(0)
{
	...
	// Deduce the server
	const char* defaultServer = "127.0.0.1:12345";

	m_dcpsInfoRepo = config.getValue(GMSEC_MIDDLEWARE_SERVER, "");

	if (m_dcpsInfoRepo.empty())
	{
		// Server not provided; check legacy configuration option
		m_dcpsInfoRepo = config.getValue(GMSEC_SERVER, defaultServer);
	}
}
...
void OpenDDSConnection::mwConnect()
{
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, INVALID_CONNECTION_TYPE,
	...
	if (errorConnecting)
	{
		throw GmsecException(MIDDLEWARE_ERROR, INITIALIZATION_ERROR, errorReason.c_str());
	}
	...
	getExternal().dispatchEvent(Connection::Event::CONNECTION_SUCCESSFUL_EVENT, status);
}
...
void OpenDDSConnection::mwDisconnect()
{
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: Exception caught in cleanup.");
	}
	...
	GMSEC_INFO << status.get();
	getExternal().dispatchEvent(Connection::Event::CONNECTION_BROKEN_EVENT, status);
}
...
void OpenDDSConnection::mwSubscribe(const char* subject, const Config &config)
{
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: register_type failed!");
	}
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: create_topic failed!");
	}
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: create_subscriber failed!");
	}

	// Create DataReader Listener
	bool dropMessages = config.getBooleanValue("__DROP_MESSAGES__", false);

	DDS::DataReaderListener_var listener(new OpenDDSMessageListener(subject, this, m_queue, dropMessages));

	if (CORBA::is_nil(listener.in()))
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: listener is nil.");
	}
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: create_datareader failed!");
	}
...
void OpenDDSConnection::mwUnsubscribe(const char* subject)
{
	...
	if (it != m_subscriptions.end())
	{
		...
		{
			throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, ret, "Error: DataReader delete failed");
		}
	}
	else
	{
...
void OpenDDSConnection::mwPublish(const Message& msg, const Config& config)
{
	...
	while (true)
	{
		...
		{
			throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: get_publication_matched_status failed!");
		}
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: _narrow failed!");
	}

	DDS::ReturnCode_t ret = message_writer->write(openddsMsg, DDS::HANDLE_NIL);
	if (ret != DDS::RETCODE_OK)
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, ret, "Error: message_writer failed to write");
	}	
...
void OpenDDSConnection::mwReceive(Message*& msg, GMSEC_I32 timeout)
{
	...
	while (!done)
	{
		...
		else
		{
			...
			{
				throw GmsecException(info.result);
			}
...
OpenDDSConnection::OpenDDSWriter* OpenDDSConnection::createWriter(const std::string& subject, const Config& config)
{
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: create_publisher failed!");
	}
	...
	{
		throw GmsecException(MIDDLEWARE_ERROR, OTHER_ERROR_CODE, "Error: create_datawriter failed!");
	}
...
OpenDDSMessage::Message OpenDDSConnection::prepareMsg(const gmsec::api5::Message& msg)
{
	std::string subject(msg.getSubject());
	...
	meta.setI32("MSG-KIND", static_cast<GMSEC_I32>(msg.getKind()));
