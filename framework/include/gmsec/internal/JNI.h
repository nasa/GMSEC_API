...
#ifndef gmsec_internal_jni_h
#define gmsec_internal_jni_h
...
public:
	...
	jmethodID getMethod(jclass klazz,
			const char *name,
			const char *signature);
	jmethodID getStaticMethod(jclass klazz,
			const char *name,
			const char *signature);
...
private:
	JNIObject(const JNIObject &); // = 0;
	JNIObject &operator=(const JNIObject &); // = 0;
...
#define JNI_CATCH(env) \
		...
		LOG_WARNING << "JNI_CATCH(std::bad_alloc): " << e.what(); \
		if (jni::checkJVM(env) && gmsec::util::stringLength(e.what()) < 200) { \
			char buffer[256]; \
		...
		LOG_WARNING << "JNI_CATCH(std::exception): " << e.what(); \
		if (jni::checkJVM(env) && gmsec::util::stringLength(e.what()) < 200) { \
			char buffer[256]; \
