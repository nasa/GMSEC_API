...
#ifndef GMSEC_API5_INTERNAL_MSGFIELDMAP_H
#define GMSEC_API5_INTERNAL_MSGFIELDMAP_H
...
public:
	 ...
	 */
	MsgFieldMap& operator=(const MsgFieldMap& other);
	 ...
	 *
	 * @throw GmsecException if field is not found.
	 */
	 ...
	 * @return Field object.
	 *
	 * @throw GmsecException is thrown if there are no more iterable fields.
	 */
...
#if defined(_MSC_VER)
	#if _MSC_VER >= 1700
	typedef std::unordered_map<std::string, gmsec::api5::Field*> HashMap;
	#else
	typedef stdext::hash_map<std::string, gmsec::api5::Field*> HashMap;
	#endif
...
#else 
	typedef std::tr1::unordered_map<std::string, gmsec::api5::Field*> HashMap;
#endif

	typedef std::map<std::string, gmsec::api5::Field*> BinaryTreeMap;
