 ...
 * GMSEC_Field  field  = u32FieldCreate("U32-FIELD", 12345, status);
 *
 * if (!field)
 * {
...
#ifndef GMSEC_API_C_U32_FIELD_H
#define GMSEC_API_C_U32_FIELD_H
...
extern "C"
{
	...
	/**
	 * @fn GMSEC_Field u32FieldCreate(const char* fieldName, GMSEC_U32 value, GMSEC_Status status)
	 *
	 ...
	 * @sa fieldDestroy()
	 */
	GMSEC_API GMSEC_Field u32FieldCreate(const char* fieldName, GMSEC_U32 value, GMSEC_Status status);


	/**
	 * @fn GMSEC_U32 u32FieldGetValue(const GMSEC_Field field, GMSEC_Status status)
	 *
	 * @note A status object MUST BE CREATED with statusCreate() and
	 * passed in as the status parameter in order to receive any resulting status.
	 *
	 ...
	 */
	GMSEC_API GMSEC_U32 u32FieldGetValue(const GMSEC_Field field, GMSEC_Status status);
