 ...
 * GMSEC_Field  field  = f32FieldCreate("F32-FIELD", 1234.0, status);
 *
 * if (!field)
 * {
...
#ifndef GMSEC_API_C_F32_FIELD_H
#define GMSEC_API_C_F32_FIELD_H
...
extern "C"
{
	...
	/**
	 * @fn GMSEC_Field f32FieldCreate(const char* fieldName, GMSEC_F32 value, GMSEC_Status status)
	 *
	 ...
	 * @sa fieldDestroy()
	 */
	GMSEC_API GMSEC_Field f32FieldCreate(const char* fieldName, GMSEC_F32 value, GMSEC_Status status);


	/**
	 * @fn GMSEC_F32 f32FieldGetValue(const GMSEC_Field field, GMSEC_Status status)
	 *
	 * @note A status object MUST BE CREATED with statusCreate() and
	 * passed in as the status parameter in order to receive any resulting status.
	 *
	 ...
	 */
	GMSEC_API GMSEC_F32 f32FieldGetValue(const GMSEC_Field field, GMSEC_Status status);
