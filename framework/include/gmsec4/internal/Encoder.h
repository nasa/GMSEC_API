...
public:
	...
	**/
	Status encode(const Message &message, GMSEC_U64 &bytes, char *&data);
	...
	**/
	Status encode(const Message &message, gmsec::api::util::DataBuffer &out);
...
private:
	...
	Status findLength(const Message &message, GMSEC_I32 &fieldCount);
	bool updateLength(const Field &field);

	Status encode(const Message &message, GMSEC_I32 fieldCount);
	bool encode(const Field &field);
...
public:
	...
	**/
	 ...
	 *
	**/
	Status decode(Message &message, const gmsec::api::util::DataBuffer &in);
...
private:
	...
	Status decodeAux(Message &message, const gmsec::api::util::DataBuffer &in);
