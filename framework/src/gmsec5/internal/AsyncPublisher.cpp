...
AsyncPublisher::AsyncPublisher(ConnectionInterface* connIf, BoundedQueue<MessagePublishTask>* asyncQueue)
	: m_connIf(connIf),
...
bool AsyncPublisher::stop(int timeout_ms)
{
	if (!m_alive.get())
	{
	...
	while ((m_alive.get() || !m_asyncQueue->empty()) && !m_hardKill.get())
	{
		if (m_asyncQueue->poll(500, mpt))
		{
			if (mpt.msg)
			{
				...
				{
					GMSEC_ERROR << "Asynchronous Publisher could not publish message. Error status: " << e.what();
				}
