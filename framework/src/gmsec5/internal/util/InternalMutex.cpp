	...
	: m_depth(0)
{
	...
	if (!m_handle)
	{
		...
		throw GmsecException(OTHER_ERROR, INITIALIZATION_ERROR, code, errorInfo(code).c_str());
	}
...
void InternalMutex::enterMutex()
{
	...
	if (code == WAIT_FAILED)
	{
		throw GmsecException(OTHER_ERROR, OTHER_ERROR_CODE, code, errorInfo(GetLastError()).c_str());
	}
	...
	if (code)
	{
		throw GmsecException(OTHER_ERROR, OTHER_ERROR_CODE, code, errorInfo(code).c_str());
	}
