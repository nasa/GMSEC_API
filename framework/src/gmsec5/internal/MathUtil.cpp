...
GMSEC_F32 MathUtil::extractF32(const char* hexstr)
{
	...
	{
		throw GmsecException(FIELD_ERROR, INVALID_FIELD_VALUE, "Field F32 BITS must be 8 hex digits");
	}
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        if (upper < 0 || lower < 0)
        {
			throw GmsecException(FIELD_ERROR, INVALID_FIELD_VALUE, "Field F32 BITS must be 32 hex digits");
        }
...
GMSEC_F64 MathUtil::extractF64(const char* hexstr)
{
	...
	{
		throw GmsecException(FIELD_ERROR, INVALID_FIELD_VALUE, "Field F64 BITS must be 16 hex digits");
	}
