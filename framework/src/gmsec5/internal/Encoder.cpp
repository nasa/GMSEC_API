...
{
	throw GmsecException(MSG_ERROR, ENCODING_ERROR, text);
}
...
MessageEncoder::MessageEncoder()
	...
	current(0),
	encoder(Encoder::getEncoder()),
	selector(MessageFieldIterator::Selector::ALL_FIELDS),
	altSelector(0)
{
...
void MessageEncoder::encode(const Field &field)
{
	...
	case Field::Type::BINARY:
	{
		...
		GMSEC_U8 const* value = binField.getValue();
		GMSEC_U32       count = static_cast<GMSEC_U32>(binField.getLength());

		encoder->putCountedBytes(reinterpret_cast<const char*>(value), count, current);
		break;
	}
	case Field::Type::I64:
	{
...
void MessageDecoder::decode(Message &message, GMSEC_U64 count, const char *data)
{
	...
	{
		DataBuffer tmp(reinterpret_cast<const DataBuffer::data_t*>(data), static_cast<int>(count), false);
...
Field* MessageDecoder::decodeField()
{
	...
	{
		setError("MessageEncoder::decode : Not enough data for field type");
	}

	GMSEC_U32 count = 0;
	if (!decoder->getString(*this, current, count, name))
	{
		setError("MessageEncoder::decode : Unable to decode field name (is the data encoded as XML or JSON?)");
	}
	...
	case Field::Type::CHAR:
	{
		...
		if (checkAvailable(1))
		{
			...
			field = new CharField(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for CHAR");
		}
		break;
	}
	case Field::Type::BOOL:
	{
		...
		if (checkAvailable(1))
		{
			...
			field = new BooleanField(name, (value ? true : false), isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for BOOL");
		}
		break;
	}
	case Field::Type::I8:
	{
		...
		{
			decoder->getI8(current, &value);
			field = new I8Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for I8");
		}
		break;
	}
	case Field::Type::U8:
	{
		...
		if (checkAvailable(1))
		{
			...
			field = new U8Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for U8");
		}
		break;
	}
	case Field::Type::I16:
	{
		...
		if (checkAvailable(2))
		{
			...
			field = new I16Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for I16");
		}
	...
	case Field::Type::U16:
	{
		...
		if (checkAvailable(2))
		{
			...
			field = new U16Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for U16");
		}
		break;
	}
	case Field::Type::I32:
	{
		...
		if (checkAvailable(4))
		{
			...
			field = new I32Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for I32");
		}
		break;
	}
	case Field::Type::U32:
	{
		...
		if (checkAvailable(4))
		{
			...
			field = new U32Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for U32");
		}
		break;
	}
	case Field::Type::I64:
	{
		...
		if (checkAvailable(8))
		{
			...
			field = new I64Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for I64");
		}
		break;
	}
	case Field::Type::U64:
	{
		...
		if (checkAvailable(8))
		{
			...
			field = new U64Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for U64");
		}
		break;
	}
	case Field::Type::F32:
	{
		...
		if (checkAvailable(4))
		{
			...
			field = new F32Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for F32");
		}
		break;
	}
	case Field::Type::F64:
	{
		...
		if (checkAvailable(8))
		{
			...
			field = new F64Field(name, value, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Not enough data for F64");
		}
		break;
	}
	case Field::Type::STRING:
	{
		...
		{
			setError("MessageEncoder::decode : Bad string for STRING");
		}
		break;
	}
	case Field::Type::BINARY:
	{
		...
		{
			field = new BinaryField(name, reinterpret_cast<const GMSEC_U8*>(value), count, isHeader);
		}
		else
		{
			setError("MessageEncoder::decode : Bad blob for BINARY");
		}
		break;
	}
	default:
