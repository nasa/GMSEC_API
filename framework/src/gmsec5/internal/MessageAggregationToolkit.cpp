...
bool MessageAggregationToolkit::isAggregatedMsg(const Message* msg)
{
...
void MessageAggregationToolkit::processAggregatedMsg(Message*& msg)
{
	...
	GMSEC_U8 const* rawData = dataFld.getValue();
	...
	for (GMSEC_U32 i = 0; i < numMsgs && dataPos < rawSize; ++i)
	{
		...
		StdUniquePtr<Message> tmpMsg(new Message());

		tmpMsg->setSubject(msg->getSubject());
		tmpMsg->setConfig(m_connection->getMessageConfig());
...
void MessageAggregationToolkit::createMsgBin(const char* subject)
{
	...
	if (!subject || std::string(subject).empty())
	{
		throw GmsecException(CONNECTION_ERROR, INVALID_SUBJECT_NAME, "Subject cannot be NULL nor an empty string");
	}
...
{
	AggregatedMessage(const char* subject)
		: msg(new Message()),
		  numAggregated(0),
		  data()
	{
		...
		// We will be using a homemade message, thus we need to set the message kind manually.
		MessageBuddy::getInternal(*msg).setKind(Message::Kind::PUBLISH);
	}
	...
	if (bin && bin->m_messages.size() > 0)
	{
		...
		for (MessageBin::MessageList::iterator it = bin->m_messages.begin(); it != bin->m_messages.end(); ++it)
		{
			...
			if (it2 == aggregatedMessages.end())
			{
				...
				aggMsg = new AggregatedMessage(cachedMsg->getSubject());
		...
		for (AggregatedMessages::iterator it = aggregatedMessages.begin(); it != aggregatedMessages.end(); ++it)
		{
			...
			catch (const GmsecException& e)
			{
				...
				m_connection->dispatchEvent(Connection::Event::MSG_PUBLISH_FAILURE_EVENT, m_publishStatus);
			}
