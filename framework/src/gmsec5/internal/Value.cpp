...
{
	MessageFieldIterator& iter = from.getFieldIterator(MessageFieldIterator::Selector::HEADER_FIELDS);
	...
	while (iter.hasNext())
	{
		...
		switch (type)
		{
			...
			default:
				GMSEC_WARNING << "Field " << name << " has unhandled header field type: " << static_cast<int>(type);
				break;
		}
	}
}
