...
/**
 * @file ResourceInfoCollector.cpp
 *
...
std::deque<ResourceInfoCollector::MainMemoryStats> ResourceInfoCollector::m_mainMemoryStatsQueue;
std::deque<ResourceInfoCollector::DiskStats>       ResourceInfoCollector::m_diskStatsQueue;
std::deque<ResourceInfoCollector::CPUStats>        ResourceInfoCollector::m_cpuStatsQueue;
std::deque<ResourceInfoCollector::NetworkStats>    ResourceInfoCollector::m_networkStatsQueue;
...
static T getNetworkValue(const std::string& interface, const std::string& metric, T defaultValue)
{
	...
	if (output)
	{
		...
		if (error)
		{
			...
			throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_NET_ERROR, oss.str().c_str());
		}
	}
	else
	{
...
void ResourceInfoCollector::reset()
{
	m_mainMemoryStatsQueue.clear();
	m_diskStatsQueue.clear();
	m_cpuStatsQueue.clear();
	m_networkStatsQueue.clear();
}


void ResourceInfoCollector::addMainMemoryStats(Message& msg, unsigned int specVersion, size_t movingAverageSamples)
{
	...
	{
		throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_MEMORY_ERROR, "Failed to query main memory stats");
	}
...
#else
	throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_MEMORY_ERROR, "Failed to query main memory stats:  unknown operating system");
#endif
...
void ResourceInfoCollector::addDiskStats(Message& msg, unsigned int specVersion, size_t movingAverageSamples)
{
	...
	while (ifs.getline(buffer, sizeof(buffer) - 1, '\n'))
	{
		...
		Tokens tokens = StringUtil::split(buffer, " \t");
	...
	while (iss.getline(buffer, sizeof(buffer) - 1, '\n'))
	{
		...
		std::string device_path     = tokens[0];
		std::string mount_directory = tokens[8];
...
#else
	throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_DISK_MEMORY_ERROR, "Failed to query disk stats: unknown operating system");
#endif
...
void ResourceInfoCollector::addCPUStats(Message& msg, unsigned int specVersion, size_t movingAverageSamples)
{
	...
	{
		throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_CPU_ERROR, "Unable to collect system CPU resource information");
	}
	...
	while (ifs.getline(buffer, sizeof(buffer) - 1, '\n'))
	{
		...
		// we want "cpu1", "cpu2", etc, but not "cpu", nor anything else
        if (tmp.find("cpu ") != std::string::npos || tmp.find("cpu") == std::string::npos) continue;
...
#else
	throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_CPU_ERROR, "Failed to query CPU stats: unknown operating system");
#endif
	...
	for (int cpu_count = 0; cpu_count < stats_this_iteration.num_cpus; ++cpu_count)
	{
		...
		if (m_cpuStatsQueue.size() > 0)
		{
			...
			GMSEC_F32 util_ticks_delta  = static_cast<GMSEC_F32>(end_util_ticks - start_util_ticks);
			GMSEC_F32 total_ticks_delta = static_cast<GMSEC_F32>(end_total_ticks - start_total_ticks);

			if (total_ticks_delta > 0)
			{
				average_cpu_utils = (util_ticks_delta / total_ticks_delta) * 100.0;
			}
		}
...
void ResourceInfoCollector::addNetworkStats(Message& msg, size_t sampleInterval, size_t movingAverageSamples)
{
	//This function is responsible for:
	//NUM-OF-NET-PORTS
	//NET-PORT.n.NAME
	...
	{
		throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_NET_ERROR, "Failed to query network stats:  failed call to getifaddrs()");
	}
...
#else
	throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_NET_ERROR, "Failed to query network stats:  unknown operating system");
#endif
	...
	for (int net_port_count = 0; net_port_count < stats_this_iteration.num_net_ports; ++net_port_count)
	{
		...
		if (stats_this_iteration.stats[net_port_count].bandwidth > 0)
		{
			...
			if (m_networkStatsQueue.size() >= 2)
			{
				...
				GMSEC_U64 delta_bytesRX = stats1.stats[net_port_count].bytesRX - stats0.stats[net_port_count].bytesRX;
				GMSEC_U64 delta_bytesTX = stats1.stats[net_port_count].bytesTX - stats0.stats[net_port_count].bytesTX;
...
std::string ResourceInfoCollector::getOSVersion()
{
#ifdef WIN32
	...
	{
		throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_MEMORY_ERROR, "Failed to query Windows version information");
	}
	...
	{
		throw GmsecException(RESOURCE_GENERATOR_ERROR, RESOURCE_INFO_OS_VERSION_ERROR, "Failed to query OS version");
	}
