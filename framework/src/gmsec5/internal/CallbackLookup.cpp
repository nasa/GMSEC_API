...
void CallbackLookup::removeCallback(const char* topic)
{
	...
	else
	{
		...
		if (!found)
		{
			throw GmsecException(CALLBACK_LOOKUP_ERROR, INVALID_CALLBACK, "No callback registered for subject pattern");
		}
	}
}
...
void CallbackLookup::removeCallback(const char* topic, Callback* cb)
{
	...
	if (topic == NULL || *topic == '\0')
	{
		...
		if (!found)
		{
			throw GmsecException(CALLBACK_LOOKUP_ERROR, INVALID_CALLBACK, "Callback is not registered");
		}
	}
	else
	{
