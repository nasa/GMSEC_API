...
const Field* MsgFieldMap::getField(const char* name) const
{
	...
	if (m_type == BINARY_TREE_MAP)
	{
		BinaryTreeMap::const_iterator it = m_binTreeMap.find(name);
	...
	{
		HashMap::const_iterator it = m_hashMap.find(name);
...
bool MsgFieldMap::clearField(const char* name)
{
	...
	if (m_type == BINARY_TREE_MAP)
	{
		BinaryTreeMap::iterator it = m_binTreeMap.find(name);
...
const Field& MsgFieldMap::next()
{
	if (!hasNext())
	{
		throw GmsecException(MSG_ERROR, ITER_INVALID_NEXT, "No more message fields.");
	}
