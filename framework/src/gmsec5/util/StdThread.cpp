...
int StdThread::start()
{
	...
	fInvoker = 0;
	int result = static_cast<int>(GMSEC_BOOL::GMSEC_FALSE);
	...
	if (tmpID)
	{
		result = static_cast<int>(GMSEC_BOOL::GMSEC_TRUE);
	}
	...
	result = (err == 0) ? static_cast<int>(GMSEC_BOOL::GMSEC_TRUE) : static_cast<int>(GMSEC_BOOL::GMSEC_FALSE);

finish:

#endif

	if (result != static_cast<int>(GMSEC_BOOL::GMSEC_TRUE))
	{
