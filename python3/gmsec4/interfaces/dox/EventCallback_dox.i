...
%feature("docstring") gmsec::api::EventCallback "
    ...
    when an event occurs in the connection object.
    Please note that because users are able to create their own
    EventCallback class, reentrancy is not guaranteed unless if they
    implement their own reentrancy rules.
    Also note that because an EventCallback can be registered to
...
%feature("docstring") gmsec::api::EventCallback::onEvent "
    ...
    Please note that if an EventCallback is registered to multiple
    connections, onEvent() can be invoked concurrently from the
    different connection threads.

    Note: DO NOT DESTROY the Connection that is passed into this
    function by the API. It is owned by the API and does not need to
    be managed by the client program. References to the Connection
    object, the Status object and the event string should not be
    stored by the client program beyond the scope of this callback
    function.

    Parameters
    ----------
    conn: A reference to the Connection object that is reporting the
        event
