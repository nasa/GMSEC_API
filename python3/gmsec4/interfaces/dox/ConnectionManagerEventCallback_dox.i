...
%feature("docstring") gmsec::api::mist::ConnectionManagerEventCallback "
    ...
    when an event (typically an error) occurs in the connection object.
    ...
    unless if they implement their own reentrancy rules. Also note that
    because a ConnectionManagerEventCallback can be registered to
    multiple connections, it can be run concurrently amongst those
    connections. Because of this, the use of an AutoMutex is suggested
    ...
    cb = MyEventCallback()
    connMgr.registerEventCallback(libgmsec_python3.Connection.REQUEST_TIMEOUT, cb)
...
%feature("docstring") gmsec::api::mist::ConnectionManagerEventCallback::onEvent "
    ...
    This function is called in response to a event after a call to
    registerEventCallback().

    Please note that if a callback is registered to multiple
    connections, onEvent() can be invoked concurrently from the
    different connection threads.

    Note: DO NOT DESTROY the ConnectionManager that is passed into
        this function by the API. It is owned by the API and does not
        need to be managed by the client program.  Also, the
        ConnectionManager object should not be stored by the client
        program beyond the scope of this callback function.

    Parameters
    ----------
    connMgr: A reference to the ConnectionManager object that is
        reporting the event
    status: A reference to a status object that contains information
        regarding the event
