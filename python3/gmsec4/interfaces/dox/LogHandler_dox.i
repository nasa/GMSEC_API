...
%feature("docstring") gmsec::api::util::LogHandler::onMessage "
    ...
    A class derrived from LogHandler needs to be registered with
    Log.registerHandler() in order to be called for a particular
    logging level.

    Because this class is user defined, it is not considered
    thread-safe unless made so by the user. The use of a AutoMutex is
