...
%feature("docstring") gmsec::api::mist::message::DeviceMessage::getDeviceIterator "
    ...
    to iterate over the Device objects stored within the DeviceMessage.
    The iterator is reset each time getDeviceIterator() is called.
    The iterator itself is destroyed when the DeviceMessage object is
    destroyed.

    Note: Only one DeviceIterator object is associated with a
        DeviceMessage object; multiple calls to getDeviceIterator()
        will return a reference to the same DeviceIterator object.
