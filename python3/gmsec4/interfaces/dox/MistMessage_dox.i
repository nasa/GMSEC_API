...
%feature("docstring") gmsec::api::mist::message::MistMessage "
    ...
    This Message object shares all of the functionality of standard
    messages, but can also be auto-populated based on how the message
    ...
    Initializes the message instance with a template determined by ID
    and spec.

    Parameters
    ----------
    subject: The subject string for the message.
    schemaID: The string used to identify the message schema in the
        GMSEC ISD. The schema ID has the format of:
        messageKind.messageType.messageSubtype (e.g. MSG.C2CX.HB).
        Not all schemaIDs contain a messageSubtype.
    spec: A reference to the specification this message's schema will
        adhere to.

    Exceptions
    ----------
    A GmsecError is thrown if Specification fails to load the template
        directory or if schemaID is not a valid ID.
    ...
    schemaID: The string used to identify the message schema in the
        GMSEC ISD. The schema ID has the format of:
        messageKind.messageType.messageSubtype (e.g.
        2016.00.GMSEC.MSG.C2CX.HB). Not all schemaIDs contain a
        messageSubtype.
    spec: A reference to the specification this message's schema will
        adhere to.
...
%feature("docstring") gmsec::api::mist::message::MistMessage::getSchemaID "
    ...
    Returns a string that identifies the schema that the Message is
    based off of.
";
