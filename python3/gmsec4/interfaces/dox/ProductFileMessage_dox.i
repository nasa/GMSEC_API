...
%feature("docstring") gmsec::api::mist::message::ProductFileMessage "
    ...
    ProductFileMessage(self, subject, responseStatus,
                       productType, productSubtype, version)
    ...
    responseStatus: RESPONSE-STATUS field to insert into a MSG PROD
        message.
    productType: PROD-TYPE field to insert into a MSG PROD message.
    productSubtype: PROD-SUBTYPE field to indert into a MSG PROD
        message.
    version: The version of the GMSEC message specification to be used.

    ProductFileMessage(self, config, subject, responseStatus,
                       productType, productSubtype, version)
    ...
    subject: The subject string for the message.
    responseStatus: RESPONSE-STATUS field to indert into a MSG PROD
        message.
    productType: PROD-TYPE field to insert into a MSG PROD message.
    productSubtype: PROD-SUBTYPE field to indert into a MSG PROD
        message.
...
%feature("docstring") gmsec::api::mist::message::ProductFileMessage::getProductFile "
    ...
    Get ProductFile numbered at index. Note that the index which will
    be retrieved does not correspond to the GMSEC ISD and starts from 0
    instead of 1.
    For example, getProductFile(0) would return the ProductFile
    ...
    Exceptions
    ----------
    A GmsecError is thrown if the index specified is not in the
        range of ProductFiles in this message.

    Note: This function has been deprecated, use ProductFileIterator
        instead.
";
...
%feature("docstring") gmsec::api::mist::message::ProductFileMessage::getProductFileIterator "
    ...
    associated with the ProductFileMessage.
    This iterator will allow for applications to iterate over the
    ProductFile objects stored within the ProductFileMessage.
    The iterator is reset each time getProductFileIterator() is called.
    The iterator itself is destroyed when the ProductFileMessage object
    is destroyed.

    Note: Only one ProductFileIterator object is associated with a
    ProductFileMessage object; multiple calls to
    getProductFileIterator() will return a reference to the same
    ProductFileIterator object.  Each call will reset the iterator.
