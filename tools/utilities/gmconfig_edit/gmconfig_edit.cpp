...
GmsecConfigEdit::GmsecConfigEdit(const gmsec::api5::Config& config)
	: Utility(config),
	  m_configFile(new gmsec::api5::ConfigFile()),
	  m_menuItems(),
	  m_unsavedChanges(false)
{
	...
	m_menuItems.push_back(MenuItem<GmsecConfigEdit>("Add a subscription definition",   this, &GmsecConfigEdit::addSubscriptionEntry));
	...
	m_menuItems.push_back(MenuItem<GmsecConfigEdit>("Remove subscription definition",  this, &GmsecConfigEdit::removeSubscriptionEntry));
...
bool GmsecConfigEdit::addSubscriptionEntry()
{
	bool success = false;
	...
	if (haveConfigFile())
	{
		...
		try
		{
			m_configFile->addSubscriptionEntry(gmsec::api5::ConfigFile::SubscriptionEntry(name.c_str(), topic.c_str()));
			success = true;
		}
		catch (const gmsec::api5::GmsecException& e)
		{
...
bool GmsecConfigEdit::removeSubscriptionEntry()
{
	bool success = false;
	...
	if (haveConfigFile())
	{
		...
		try
		{
			success = m_configFile->removeSubscriptionEntry(name.c_str());
...
bool GmsecConfigEdit::findConfiguration()
{
	...
	if (haveConfigFile())
	{
		...
		try
		{
			gmsec::api5::Config config = m_configFile->lookupConfig(name.c_str());
			success = true;
		}
		catch (const gmsec::api5::GmsecException& e)
		{
...
bool GmsecConfigEdit::addMessage()
{
	...
	if (haveConfigFile())
	{
		...
		gmsec::api5::Message::MessageKind msgKind;
		if (type == "PUBLISH")
		{
			msgKind = gmsec::api5::Message::PUBLISH;
		}
		else if (type == "REQUEST")
		{
			msgKind = gmsec::api5::Message::REQUEST;
		}
		else if (type == "REPLY")
		{
			msgKind = gmsec::api5::Message::REPLY;
		}
		else
		{
		...
		gmsec::api5::Message msg(topic.c_str(), msgKind);
#endif
		...
		for (size_t n = 0; n < numFields; ++n)
		{
			...
			else if (fieldType == "BIN")
			{
				...
				msg.addField(fieldName.c_str(), reinterpret_cast<const GMSEC_U8*>(value.c_str()), value.size());
			}
			else
			{
...
bool GmsecConfigEdit::findMessage()
{
	...
	if (haveConfigFile())
	{
		...
		try
		{
			gmsec::api5::Message msg = m_configFile->lookupMessage(name.c_str());
			success = true;
		}
		catch (const gmsec::api5::GmsecException& e)
		{
