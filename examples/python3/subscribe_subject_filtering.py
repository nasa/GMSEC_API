...
""" 
 @file subscribe_subject_filtering.py
...
EXAMPLE_SUBSCRIPTION_SUBJECT = "GMSEC.>"
...
def main():
    ...
    try:
        ...
        # filter
        libgmsec_python3.log_info("Adding a filter rule for the topic: " + FILTERED_MESSAGE_SUBJECT)
        connMgr.exclude_subject(FILTERED_MESSAGE_SUBJECT)
        ...
        message = connMgr.receive(libgmsec_python3.GMSEC_WAIT_FOREVER)
        ...
        libgmsec_python3.log_info("Waiting 5 seconds to demonstrate that a second message will not be received")
        message = connMgr.receive(5000)

        if (message != None):
            libgmsec_python3.log_error("Unexpectedly received a filtered message:\n" + message.to_XML())
