...
def createMsgUsingConfigFile(filename):
    ...
    cfg = cfgFile.lookup_config("config")
    ...
    for i in range(0,3):
        ...
        msg = cfgFile.lookup_message(msgName)
...
def createMsgUsingConfigObject():
    ...
    cfg1 = libgmsec_python3.Config()
    cfg2 = libgmsec_python3.Config()
    cfg3 = libgmsec_python3.Config()
    empty = libgmsec_python3.Config()

    cfg1.add_value("gmsec-msgfld-store-type", "tree")
    ...
    cfg1.add_value("gmsec-msgfld-store-size", "10")

    cfg2.add_value("gmsec-msgfld-store-type", "hash")

    cfg3.add_value("gmsec-msgfld-store-type", "tree")    # container type
    cfg3.add_value("gmsec-msgfld-store-size", "0")      # never rollover
...
def buildMessage(config, numFields):
    ...
    msg = libgmsec_python3.Message("GMSEC.SYSTEST.TEST.REQ.DIR.LONG", libgmsec_python3.Message.PUBLISH, config)
