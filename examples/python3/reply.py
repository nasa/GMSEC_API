...
class reply:
    def __init__(self):
        ...
        if level == lp.Specification.SchemaLevel_LEVEL_0:
            self.subscription_topic = "C2MS.*.*.MY-MISSION.*.MY-SAT-ID.REQ.DIR.REPLY"

        elif level == lp.Specification.SchemaLevel_LEVEL_1:
            self.subscription_topic = "C2MS-PIPE.*.*.MY-MISSION.*.MY-SAT-ID.REQ.DIR.REPLY"

        else:
            self.subscription_topic = "GMSEC.MY-MISSION.MY-SAT-ID.REQ.DIR.REPLY"
