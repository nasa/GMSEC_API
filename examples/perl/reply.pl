...
sub main
{
    ...
    {
        $subscriptionTopic = "C2MS.*.*.MY-MISSION.*.MY-SAT-ID.REQ.DIR.REPLY";
    }
    elsif ($level == $libgmsec_perl::Specification::SchemaLevel_LEVEL_1)
    {
        $subscriptionTopic = "C2MS-PIPE.*.*.MY-MISSION.*.MY-SAT-ID.REQ.DIR.REPLY";
    }
    else
    {
        $subscriptionTopic = "GMSEC.MY-MISSION.MY-SAT-ID.REQ.DIR.REPLY";
    }
