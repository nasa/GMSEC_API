...
namespace gmpub_wsmq_async
{
	...
	class AsyncStatusCheckCallback : EventCallback
	{
		public override void OnEvent(Connection conn, Status status, Connection.ConnectionEvent evt)
		{
	...
	class gmpub_wsmq_async : GmsecExample
	{
		private Config     config;
		private Connection conn;


		public gmpub_wsmq_async()
		{
		}
		...
		{
			Log.Error("\nusage: gmpub_wsmq_async_cs connectiontype=<middleware> " +
			          "[ <parameter>=<value> ]\n" +
		...
		private void Run(string[] args)
		{
			...
			try
			{
                ...
                conn.RegisterEventCallback(Connection.ConnectionEvent.WSMQ_ASYNC_STATUS_CHECK_EVENT, new AsyncStatusCheckCallback());
