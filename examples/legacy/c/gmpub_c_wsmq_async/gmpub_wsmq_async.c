...
void gmpub_wsmq_async_printUsage()
{
	printf("\nusage: gmpub_wsmq_async_c connectiontype=<middleware> "
		"[ <parameter>=<value> ]\n"
...
void AsyncStatusCheckCallback(GMSEC_CONNECTION_OBJECT conn,
	GMSEC_MESSAGE_OBJECT msg,
	GMSEC_STATUS_OBJECT status,
	GMSEC_STR event)
{
	LOG_INFO("%s", gmsec_GetStatus(status));

	if (gmsec_isStatusError(status))
	{
		LOG_ERROR("The first occurrence of a WebSphere MQ Asynchronous Put Response warning or failure returned the WebSphere Reason Code: %d", gmsec_GetStatusCustomCode(status));
	}
}

int gmpub_wsmq_async_Initialize(gmpub_wsmq_async_t *this, int argc, char *argv[])
{
	this->status = NULL;
...
int gmpub_Publish(gmpub_wsmq_async_t *this, int iteration, const char *subject)
{
	GMSEC_STR xml;
	...
	/* Create a Message */
	gmsec_CreateMessage(this->connection, subject, GMSEC_MSG_PUBLISH, &(this->message), this->status);
	...
	gmsec_SetFieldValueF32(this->field, (GMSEC_F32) (1 + 1. / value), this->status);
	gmsec_MsgAddField(this->message, this->field, this->status);
	if (!example_check("AddField(<GMSEC_F32>)", this->status)) return GMSEC_FALSE;

	gmsec_SetFieldName(this->field, "D", this->status);
	gmsec_SetFieldValueF64(this->field, (GMSEC_F64) (1 + 1. / value), this->status);
	gmsec_MsgAddField(this->message, this->field, this->status);
...
int gmpub_wsmq_async_Run(gmpub_wsmq_async_t *this)
{
	GMSEC_STR subject;
	GMSEC_STR tmp;
	int iterations;
	...
	LOG_INFO("Registering Async Status Check error callback");
	gmsec_RegisterErrorCallback(this->connection, "WSMQ_ASYNC_STATUS_CHECK_EVENT", &AsyncStatusCheckCallback, this->status);
	if (!example_check("RegisterErrorCallback(StatusCheck)", this->status))
	{
		return GMSEC_FALSE;
	}
...
void gmpub_wsmq_async_Cleanup(gmpub_wsmq_async_t *this)
{
	/* Destroy Field */
	if (this->field != NULL)
	{
...
{
	gmpub_wsmq_async_t gmpub_wsmq_async;

	if (!gmpub_wsmq_async_Initialize(&gmpub_wsmq_async, argc, argv))
	{
		fprintf(stderr, "\nFailure of initializing the example");
		return -1;
	}

	if (example_option_invalid(gmpub_wsmq_async.config, argc)) 
	{
		gmpub_wsmq_async_printUsage();
		return -1;
	}

	gmpub_wsmq_async_Run(&gmpub_wsmq_async);

	gmpub_wsmq_async_Cleanup(&gmpub_wsmq_async);
