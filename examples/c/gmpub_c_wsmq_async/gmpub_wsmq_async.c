...
void AsyncStatusCheckCallback(GMSEC_Connection conn, const GMSEC_Status status, GMSEC_ConnectionEvent event)
{
	GMSEC_INFO("%s", statusGetReason(status));
...
GMSEC_BOOL gmpub_wsmq_async_Initialize(gmpub_wsmq_async_t* this, int argc, char* argv[])
{
...
GMSEC_BOOL gmpub_wsmq_async_Publish(gmpub_wsmq_async_t *this, int iteration, const char *subject)
{
...
GMSEC_BOOL gmpub_wsmq_async_Run(gmpub_wsmq_async_t* this)
{
	...
	connectionRegisterEventCallback(this->connection, GMSEC_WSMQ_ASYNC_STATUS_CHECK_EVENT, AsyncStatusCheckCallback, this->status);
	if (!example_check("connectionRegisterEventCallback", this->status)) return GMSEC_FALSE;

	for (i = 0; i < iterations; ++i)
	{
		if (!gmpub_wsmq_async_Publish(this, i, subject)) return GMSEC_FALSE;
...
int main(int argc, char* argv[])
{
	...
	if (gmpub_wsmq_async_Initialize(&gmpub_wsmq_async, argc, argv) == GMSEC_FALSE)
	{
		fprintf(stderr, "\nFailure of initializing the example");
		return -1;
	}

	if (example_option_invalid(gmpub_wsmq_async.config, argc) == GMSEC_TRUE)
	{
