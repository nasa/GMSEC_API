...
void CxxLogHandlerProxy::onMessage(const LogEntry& entry)
{
	...
	if (jenv)
	{
		if (!jLogHandler)
		{
			...
			LogEntry warning = { __FILE__, __LINE__, LogLevel::logWARNING, TimeUtil::getCurrentTime(), "No reference to JNI Log Handler is available" };
		...
		switch (entry.level)
		{
			case LogLevel::logNONE:    levelField = Cache::getCache().fieldLogLevelNONE; break;
			case LogLevel::logERROR:   levelField = Cache::getCache().fieldLogLevelERROR; break;
			case LogLevel::logSECURE:  levelField = Cache::getCache().fieldLogLevelSECURE; break;
			case LogLevel::logWARNING: levelField = Cache::getCache().fieldLogLevelWARNING; break;
			case LogLevel::logINFO:    levelField = Cache::getCache().fieldLogLevelINFO; break;
			case LogLevel::logVERBOSE: levelField = Cache::getCache().fieldLogLevelVERBOSE; break;
			case LogLevel::logDEBUG:   levelField = Cache::getCache().fieldLogLevelDEBUG; break;
	...
	else
	{
		...
		LogEntry warning = { __FILE__, __LINE__, LogLevel::logWARNING, TimeUtil::getCurrentTime(), "Unable to log to Java log handler" };
...
JNIEXPORT jlong JNICALL Java_gov_nasa_gsfc_gmsec_api5_jni_gmsecJNI_new_1LogHandler
  (JNIEnv *jenv, jclass jcls, jobject logHandler)
{
	LogHandler* proxy = new CxxLogHandlerProxy(jenv, logHandler);

	return JNI_POINTER_TO_JLONG(proxy);
}


JNIEXPORT void JNICALL Java_gov_nasa_gsfc_gmsec_api5_jni_gmsecJNI_delete_1LogHandler
