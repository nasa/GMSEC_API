...
JNIEXPORT jlong JNICALL Java_gov_nasa_gsfc_gmsec_api_jni_gmsecJNI_new_1DeviceMessage__Ljava_lang_String_2I
  (JNIEnv *jenv, jclass jcls, jstring jSubject, jint jVersion)
{
	...
	try
	{
		...
		if (jvmOk(jenv, "DeviceMessage"))
		{
			try
			{
				DeviceMessage* devMsg = new DeviceMessage(subject.c_str(), (unsigned int) jVersion);
...
JNIEXPORT jlong JNICALL Java_gov_nasa_gsfc_gmsec_api_jni_gmsecJNI_new_1DeviceMessage__Ljava_lang_String_2JLgov_nasa_gsfc_gmsec_api_jni_JNIConfig_2I
	...
	try
	{
		...
		else
		{
			...
			if (jvmOk(jenv, "DeviceMessage"))
			{
				try
				{
					DeviceMessage* devMsg = new DeviceMessage(subject.c_str(), *config, (unsigned int) jVersion);
  ...
  (JNIEnv *jenv, jclass jcls, jlong jDevMsgPtr, jobject jDevMsg)
{
	...
	try
	{
		DeviceMessage* other = JNI_JLONG_TO_DEVICE_MSG(jDevMsgPtr);
...
JNIEXPORT void JNICALL Java_gov_nasa_gsfc_gmsec_api_jni_gmsecJNI_delete_1DeviceMessage
  (JNIEnv *jenv, jclass jcls, jlong jDevMsgPtr, jobject jDevMsg)
{
	try
	{
...
JNIEXPORT void JNICALL Java_gov_nasa_gsfc_gmsec_api_jni_gmsecJNI_DeviceMessage_1AddDevice
  (JNIEnv *jenv, jclass jcls, jlong jDevMsgPtr, jobject jDevMsg, jlong jDevicePtr, jobject jDevice)
{
	try
	{
		DeviceMessage* devMsg = JNI_JLONG_TO_DEVICE_MSG(jDevMsgPtr);
